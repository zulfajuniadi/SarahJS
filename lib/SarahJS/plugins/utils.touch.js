define(['sarah', 'lodash', 'fastclick'], function(app, _){
    var touch;
    var target;
    var timeStamp;
    var Utils = app.Utils;
    ...
    fn.touch = function(selector) {
        var elements = document.querySelectorAll(selector);
        if(elements.length > 0) {
            for(i = 0; i < elements.length; i++) {
                elements[i].addEventListener('touchstart', doTouchStart, false);
                elements[i].addEventListener('touchmove', doTouchEnd, false);
                FastClick.attach(elements[i]);
            }
        }
    }

    fn.untouch = function(selector) {
        var elements = document.querySelectorAll(selector);
        if(elements.length > 0) {
            for(i = 0; i < elements.length; i++) {
                elements[i].removeEventListener('touchstart', doTouchStart);
                elements[i].removeEventListener('touchmove', doTouchEnd);
            }
        }
    }
